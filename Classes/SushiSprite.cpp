...
static const char *sushiFivecrush = "sushiFC.png";
...
void SushiSprite::setDisplayMode(DisplayMode mode)
{
	...
	SpriteFrame *frame;
	switch (mode) {
	case DISPLAY_MODE_FIVECRUSH:
		frame = SpriteFrameCache::getInstance()->getSpriteFrameByName(sushiFivecrush);
		break;
